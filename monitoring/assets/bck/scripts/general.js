...
function login() {
    ...
    xmlHttp.open("GET", "void.php?par[mode]=lgn&inp[username]=" + username.value + "&inp[password]=" + password.value, true);
    xmlHttp.send(null);
    return false;
}
...
function addNol(value) {
    return (value < 10) ? `0${value}` : value;
}

function createCookieOutlet(name, value, days) {
    if (days) {
        var date = new Date();
        date.setTime(date.getTime() + (days * 24 * 60 * 60 * 1000));
        var expires = "; expires=" + date.toGMTString();
    } else var expires = "";
    document.cookie = name + "=" + value + expires + "; path=/monitoring";
}

function setCookieOutlet(elem) {
    jQuery.cookie("cIDOutlet", elem, {path: '/monitoring'});
}
