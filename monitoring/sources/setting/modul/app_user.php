...
function tambah(){
	...
	repField();
	$fotoUser = fotoUser();
	$sql="insert into app_user (username, kodeGroup, idPegawai, password, namaUser, keteranganUser, fotoUser, statusUser, createBy, createTime) values ('$inp[username]', '$inp[kodeGroup]', '$inp[idPegawai]', '".md5($inp[password])."', '$inp[namaUser]', '$inp[keteranganUser]', '$fotoUser', '$inp[statusUser]', '$cUsername', '".date('Y-m-d H:i:s')."')";
	db($sql);
...
<div id=\"contentwrapper\" class=\"contentwrapper\">
	<form action=\"\" method=\"post\" class=\"stdform\">
		...
		//if(!empty($par[filter]))
					...
					$sql="select * from app_user t1 join app_group t2 join app_group_modul t3 on (t1.kodeGroup=t2.kodeGroup and t2.kodeGroup=t3.kodeGroup) $filter order by t1.statusUser desc, t1.username ";
					// echo $sql;
					$res=db($sql);		
