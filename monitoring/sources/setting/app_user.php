...
function ubah(){
	global $s,$inp,$par,$cUsername, $det, $unit;		
	repField();
	$fotoUser = fotoUser();
	$sql="update app_user set username='$inp[username]', kodeGroup='$inp[kodeGroup]', idPegawai='$inp[idPegawai]', namaUser='$inp[namaUser]', keteranganUser='$inp[keteranganUser]', fotoUser='$fotoUser', jabatan='$inp[jabatan]', hp='$inp[hp]', statusUser='$inp[statusUser]', updateBy='$cUsername', updateTime='".date('Y-m-d H:i:s')."' where username='$par[username]'";
	db($sql);				
	...
	db("delete from app_user_unit where username='$par[username]'");
	if (is_array($unit)) {
		while (list($kodeUnit) = each($unit)) {     
			$sql = "insert into app_user_unit (kodeUnit, username, createBy, createTime) values ('$kodeUnit', '$par[username]', '$cUsername', '" . date('Y-m-d H:i:s') . "')";
			db($sql);      
		}
	} 
...
function tambah(){
	...
	$password = md5($pengacak2.$pengacak.md5($password).$pengacak.$pengacak2.$pengacak.$pengacak2);
	$sql="insert into app_user (username, kodeGroup, idPegawai, password, namaUser, keteranganUser, fotoUser, jabatan, hp, statusUser, createBy, createTime) values ('$inp[username]', '$inp[kodeGroup]', '$inp[idPegawai]', '".$password."', '$inp[namaUser]', '$inp[keteranganUser]', '$fotoUser', '$inp[jabatan]', '$inp[hp]', '$inp[statusUser]', '$cUsername', '".date('Y-m-d H:i:s')."')";
	db($sql);
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">	
			<ul class=\"hornav\" style=\"margin: 0; margin-top: 10px;\">
				...
				<li><a href=\"#unit_bisnis\">Unit Bisnis</a></li>-->
			</ul>
		...
		<p>
			<label class=\"l-input-small\">User Group</label>
			<div class=\"field\">
		...
		<p>
			...
			<p>
				...
				<div class=\"field\">
					<input type=\"text\" id=\"inp[jabatan]\" name=\"inp[jabatan]\"  value=\"$r[jabatan]\" class=\"smallinput\" style=\"width:220px;\"/>
				</div>
			</p>
			<p>
				<label class=\"l-input-small\">No HP</label>
				<div class=\"field\">
					<input type=\"text\" id=\"inp[hp]\" name=\"inp[hp]\"  value=\"$r[hp]\" class=\"smallinput\" style=\"width:220px;\"/>
				</div>
			</p>
			<p>
		...
		<div id=\"area_akses\" class=\"subcontent\" style=\"display:none;\">
			...
			<div id=\"unit_bisnis\" class=\"subcontent\" style=\"display:none;\">
			<p>
				<label class=\"l-input-small\">Unit Bisnis</label>
				<div class=\"field\" style=\"margin-left:175px;\">";

					$unit = arrayQuery("select kodeUnit, username from app_user_unit where username='$par[username]'");
					$sql__="select * from mst_data where statusData='t' AND kodeCategory = 'W03' order by urutanData";	
					$res__=db($sql__);
					while($r__=mysql_fetch_array($res__)){
						$checked = isset($unit["$r__[kodeData]"]) ? "checked=\"checked\"" : "";
						$text.="<input type=\"checkbox\" id=\"unit[".$r__[kodeData]."]\" name=\"unit[".$r__[kodeData]."]\" value=\"".$r__[kodeModul]."\" $checked /> ".$r__[namaData]."<br>";
					}
					$text.="</div>
				</p>
			</div>
...
<div id=\"contentwrapper\" class=\"contentwrapper\">
	<form action=\"\" method=\"post\" class=\"stdform\">
		...
		//if(!empty($par[filter]))	
					...
					$sql="select * from app_user t1 join app_group t2 on (t1.kodeGroup=t2.kodeGroup) $filter and t1.jenisUser !='1' order by t1.statusUser desc, t1.username";
					$res=db($sql);		
				...
				<div id=\"contentwrapper\" class=\"contentwrapper\">
					...
					<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dyntable\">
						...
						<tbody>";
							...
							$filter = "where reg_no is not null AND cat !='533'";
