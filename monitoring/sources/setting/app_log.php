	...
	function lihat(){
		...
		$text.="<div class=\"pageheader\">
			...
			<table style=\"width:100%; margin-bottom:5px;\">
				<tr>
				<td>".comboMonth("par[bulan]", $par[bulan], "onchange=\"document.getElementById('form').submit();\"")." ".comboYear("par[tahun]", $par[tahun], "", "onchange=\"document.getElementById('form').submit();\"")."
	".comboData("select * from app_user  order by namaUser","username","namaUser","par[user]","All",$par[user],"onchange=\"document.getElementById('form').submit();\"","210px;")." ".comboData("select * from log_access where aktivitasLog !='' group by aktivitasLog order by aktivitasLog","aktivitasLog","aktivitasLog","par[aktivitasLog]","All",$par[aktivitasLog],"onchange=\"document.getElementById('form').submit();\"","110px;");
	$text.="				
...
if(!empty($par[aktivitasLog])) $filter.=" and t1.aktivitasLog='$par[aktivitasLog]'";
		...
		while($r=mysql_fetch_array($res)){	
			...
			$text.="<tr>
					...
					<td>		<a href=\"#detail\" title=\"Edit Data\"   onclick=\"openBox('popup.php?par[mode]=det&par[kodeLog]=$r[kodeLog]" . getPar($par, "mode,kodeLog") . "',1000,500);\"><span>".aktivitasLog($r[aktivitasLog])."</span></a></td>
	...
	function detail(){
		...
		$sql="select * from log_access t1 join log_access_detail t2 on (t1.kodeLog = t2.kodeLog) where t2.kodeLog = '$par[kodeLog]' order by t1.createTime desc";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){	
			$no++;
			list($tanggalCreate, $waktuCreate) = explode(" ", $r[createTime]);
