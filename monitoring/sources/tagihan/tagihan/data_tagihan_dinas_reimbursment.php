...
function generateNoTiket()
{
    ...
    if ($arrParam[$s] == '1048') $str = "V";
    if ($arrParam[$s] == '1049') $str = "R";
    if ($arrParam[$s] == '1050') $str = "D";
...
function lData()
{
    ...
    if (!empty($_GET['fSearch'])) {
        ...
        or
        lower(d.nama) like '%" . mysql_real_escape_string(strtolower($_GET['fSearch'])) . "%'
        )";
    }
    ...
    $sql = "SELECT 
            ...
            LEFT JOIN pegawai_data AS d ON (d.id = b.id_supplier)
    ...
    $json = array(
        ...
        "iTotalDisplayRecords" => getField("SELECT count(*) FROM tagihan_termin AS a
                                            JOIN tagihan_spk AS b ON (b.id = a.id_spk AND b.approve_status = 't' AND b.persen_termin = '100')
                                            LEFT JOIN tagihan_data AS c ON (c.id_termin = a.id)
                                            LEFT JOIN pegawai_data AS d ON (d.id = b.id_supplier) $where"),
        "aaData" => array()
        );
    ...
    while ($r = mysql_fetch_array($res)) {
        ...
        $kontrol = "";
        if (isset($menuAccess[$s]["edit"])) $kontrol .= "<a href=\"#\" onclick=\"openBox('popup.php?par[mode]=formTiket&par[id_spk]=".$r["id_spk"]."&par[id_termin]=".$r["idTermin"]."&par[id_supplier]=".$r["idSupplier"] . getPar($par, "mode, id_spk") . "',  980, 500);\" class=\"edit\"><span>Edit</span></a>";
        ...
        $nama = getField("select nama from pegawai_data where id = '$r[id_supplier]'");
                    ...
                    <table style=\"width:100%\">
                            <td style=\"width:50%\">
                                <p>
                                    ...
                                    <div class=\"field\">
                                        ".comboData("select * from pegawai_data order by nama asc", "id", "nama", "combo3", "All", $combo3, "", "250px", "chosen-select")."
                                    </div>
		...
		<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dataList\">
            <thead>
                <tr>
                    ...
                    <th style=\"vertical-align: middle; min-width: 30px;\">Kontrol</th>
                </tr>
            </thead>
            <tbody></tbody>
        </table>
...
function xls()
{
    ...
    if (!empty($par['fSearch'])) {
        ...
        or
        lower(d.nama) like '%" . mysql_real_escape_string(strtolower($par['fSearch'])) . "%'
        )";
    }
    ...
    $sql = "SELECT 
            ...
            JOIN tagihan_spk AS b ON (b.id = a.id_spk AND b.approve_status = 't' AND b.persen_termin = '100')
            LEFT JOIN tagihan_data AS c ON (c.id_termin = a.id)
            LEFT JOIN pegawai_data AS d ON (d.id = b.id_supplier) $where order by $order";
