function chk(getPar){
	...
	xmlHttp.open("GET", "void.php?par[mode]=cek&inp[nomorSupplier]=" + nomorSupplier + getPar, true);
	xmlHttp.send(null);
	return false;
}
...
function update(getPar){
	...
	xmlHttp.open("POST","void.php?par[mode]=update" + getPar,true);
...
function getKota(getPar){
	...
	xmlHttp.open("GET", "void.php?par[mode]=kta&par[kodePropinsi]="+ kodePropinsi.value + getPar, true);
	xmlHttp.send(null);
	return false;
}
		...
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){
			...
			if(xmlHttp.responseText){
				...
				for(var i=0; i<arr.length; i++){
					...
					jQuery("#inp\\[kodeKategori\\]").trigger("chosen:updated");
				}
			}
		}
	}
	xmlHttp.open("GET", "void.php?par[mode]=subk&par[kodeProduk]="+ kodeProduk.value + getPar, true);
	xmlHttp.send(null);
	return false;
}
...
function setProduk(kodeProduk, kodeKategori, getPar){
	...
	xmlHttp.open("GET", "void.php?par[mode]=setProduk&par[kodeProduk]=" + kodeProduk + "&par[kodeKategori]=" + kodeKategori + getPar, true);
	xmlHttp.send(null);
	return false;
}
...
function setGeocode(getPar) {
	...
	xmlHttp.open("GET", "void.php?par[mode]=geo&par[kodeKota]="+ kodeKota + getPar, true);
	xmlHttp.send(null);
	return false;
}
