...
function form()
{
    ...
    setValidation("is_null", "inp[nik]", "anda harus mengisi nip");
    ...
    setValidation("is_null", "inp[bagian]", "anda harus mengisi bagian");
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1" . getPar($par) . "\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
			...
			<table style=\"width:100%\">
				<tr>
					<td style=\"width:50%\">
                        <p>
                            <label class=\"l-input-small2\">NIP</label>
                            <div class=\"fieldA\">
			...
			<p>
                ...
                <div class=\"field\">
                    " . comboData("select * from mst_data where kodeCategory = 'MDPB' order BY namaData asc", "kodeData", "namaData", "inp[bagian]", "- Pilih Bagian -", $r["bagian"], "", "610px", "chosen-select") . "
                </div>
                <style>
                    #inp_bagian__chosen{ min-width:210px; }
                </style>
            </p>
            ...
            <p>
				...
				<div class=\"field\">
					<input type=\"text\" id=\"inp[alamat]\" name=\"inp[alamat]\"  value=\"" . $r["alamat"] . "\" class=\"mediuminput\" style=\"width:600px;\" maxlength=\"50\"/>
				</div>
			</p>
			<p>
        ...
        <td style=\"width:50%\">
            ...
            <p>
				<label class=\"l-input-small\" >Nama Pemilik Rekening</label>
				<div class=\"field\">
					<input type=\"text\" id=\"inp[nama_pemilik]\" name=\"inp[nama_pemilik]\"  value=\"" . $r["nama_pemilik"] . "\" class=\"mediuminput\" style=\"width:600px;\" maxlength=\"50\"/>
				</div>
			</p>
...
function lData()
{
    ...
    $sql = "SELECT
            ...
            LEFT JOIN mst_data AS b ON (b.kodeData = a.unit)
            LEFT JOIN mst_data AS c ON (c.kodeData = a.jenis)
    ...
    $json = array(
        ...
        "iTotalDisplayRecords" => getField("SELECT count(a.id) FROM pegawai_data AS a 
                                            LEFT JOIN mst_data AS b ON (b.kodeData = a.unit)
                                            LEFT JOIN mst_data AS c ON (c.kodeData = a.jenis) $where"),
        "aaData" => array()
    );
...
function xls()
{
    ...
    if (!empty($par['fSearch'])) {
        ...
        or
        lower(nik) like '%" . mysql_real_escape_string(strtolower($par['fSearch'])) . "%'
        )";
    }

    if (!empty($par['combo1'])) {
        $where .= " and jabatan = '$par[combo1]'";
    }

    if (!empty($par['combo2'])) {
        $where .= " and jenis = '$par[combo2]'";
    }

    if (!empty($par['combo3'])) {
        $where .= " and unit = '$par[combo3]'";
    }
