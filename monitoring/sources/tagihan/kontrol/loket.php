...
function getContent($par)
{
    ...
    switch ($par[mode]) {
        ...
        case "mapping":
            $text = empty($_submit) ? form_mapping() : simpan_mapping();
            break;
...
{
    return $str.date('m').generateRandomString(4, 'num');
}
...
function print_dokumen()
{
    ...
    $vendor = getRow("SELECT a.id, b.*, c.id, b.id_jenis
                            FROM
                        tagihan_termin AS a
                        JOIN tagihan_spk AS b ON (b.id = a.id_spk AND approve_status = 't' AND persen_termin = '100' AND status_syarat = 't')
                        JOIN tagihan_data AS c ON (c.id_termin = a.id)
                             where c.id = '". $par[id_tagihan] ."'");
    if ($vendor['id_jenis'] == 1048)
    {
        $pc2 = getRow("SELECT
                        a.id,
                        c.id AS id_tagihan,
        ...
        $pc2 = getRow("SELECT
                        ...
                        JOIN tagihan_spk AS b ON (b.id = a.id_spk AND approve_status = 't' AND persen_termin = '100' AND status_syarat = 't')
                        JOIN tagihan_data AS c ON (c.id_termin = a.id)
                        JOIN pegawai_data as d ON (d.unit = b.id_sbu)
            ...
            <div style='font-size: 14px; padding: 10px; margin-top: -20px;'>
				...
				<table width='100%'>
                    ...
                    <tr>
				        ...
				        <td>:</td>
				        <td>Treasury Manager PT Patra Drilling Contractor</td>
                    </tr>
                    <tr>
                        <td width='20%'>Dari</td>
				        <td>:</td>
				        <td>Fungsi Pengaju/Expenditure Authority</td>
                    </tr>
                </table>
                ...
                <strong>Jumlah Pembayaran : IDR ". getAngka($pc1['nilai_total'])." </strong>
                ...
                <table width='100%'>
				    <tr>
				        <td width='30%'>Nama Vendor / Pekerja</td>
				        <td>:</td>
...
function simpan_mapping()
{
    ...
    $sql = "UPDATE
                ...
                SET
                  ...
                  `pengajuan_no_tiket` = '$inp[pengajuan_no_tiket]',
                  `pengajuan_penerima` = '$inp[pengajuan_penerima]',
                  `document_type` = '$inp[document_type]',
                  `document_kr_no` = '$inp[document_kr_no]',
                  `document_kz_no` = '$inp[document_kz_no]',
                  `updated_at` = now(),
                  `updated_by` = '" . $cID . "'
...
{
    global $par, $cID, $menuAccess, $s, $arrTitle;
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1" . getPar($par) . "\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
			...
			<fieldset>
                ...
                <p>
                    <label class=\"l-input-small\">No Tiket</label>
                    <div class=\"field\">   
                        ";
                        $termin["pengajuan_no_tiket"] = empty($termin["pengajuan_no_tiket"]) ? generateNoTiket("AP") : $termin["pengajuan_no_tiket"];
                        $text.="
                        <input type=\"text\" id=\"inp[pengajuan_no_tiket]\" name=\"inp[pengajuan_no_tiket]\"  value=\"" . $termin["pengajuan_no_tiket"] . "\" class=\"mediuminput\" style=\"width:300px;\" maxlength=\"50\"/>
                    </div>
                </p>
                ...
                <p>
                    ...
                    <div class=\"field\">
                        <input type=\"text\" id=\"inp[pengajuan_penerima]\" name=\"inp[pengajuan_penerima]\"  value=\"" . $termin["pengajuan_penerima"] . "\" class=\"mediuminput\" style=\"width:300px;\" maxlength=\"50\"/>
                    </div>
                </p>
                ...
                <p>
                    ...
                    <div class=\"field\">
                        <input type=\"text\" id=\"inp[document_type]\" name=\"inp[document_type]\"  value=\"" . $termin["document_type"] . "\" class=\"mediuminput\" style=\"width:300px;\" maxlength=\"50\"/>
                    </div>
                </p>
                
                <p>
                    <label class=\"l-input-small\">Document KR No.</label>
                    <div class=\"field\">
                        <input type=\"text\" id=\"inp[document_kr_no]\" name=\"inp[document_kr_no]\"  value=\"" . $termin["document_kr_no"] . "\" class=\"mediuminput\" style=\"width:300px;\" maxlength=\"50\"/>
                    </div>
                </p>
                
                <p>
                    <label class=\"l-input-small\">Document KZ No.</label>
                    <div class=\"field\">
                        <input type=\"text\" id=\"inp[document_kz_no]\" name=\"inp[document_kz_no]\"  value=\"" . $termin["document_kz_no"] . "\" class=\"mediuminput\" style=\"width:300px;\" maxlength=\"50\"/>
                    </div>
                </p>
                ...
                <p>
                    ...
                    <style>
                        #inp_pengajuan_prep_by__chosen{ min-width:210px; }
                    </style>
                </p>
...
function detailForm()
{
    ...
    $text.="
            ...
            ".view_permohonan($arrTitle[$s], $tagihan['id_spk'], '', false, true, $tagihan['id_termin'])."  
...
//        $mapping = "<img src=\"assets/images/p.png\" title='Belum dimapping'>";
//        if ($r["pengajuan_prep_by"] != "") $mapping = "<img src=\"assets/images/t.png\" title='Sudah dimapping'>";
//
//        $mapping = "<a href=\"#\" onclick=\"openBox('popup.php?par[mode]=mapping&par[id_termin]=" . $r["id"] . getPar($par, "mode, id_termin") . "', 800, 400);\">$mapping</a>";



        if ($r["pengajuan_no_tiket"] != ""){ #ini kalau full
            $background = "class=\"labelStatusHijau\"";
            $noTiket = $r["pengajuan_no_tiket"];
        }else { #kalo 0
        ...
        $kontrol = "";
        if (isset($menuAccess[$s]["edit"])) $kontrol .= "<a href=\"#\" onclick=\"openBox('popup.php?par[mode]=mapping&par[id_termin]=" . $r["id"] . getPar($par, "mode, id_termin") . "', 800, 300);\" class=\"edit\"><span>Detail</span></a>";
        ...
        $data = array(
            ...
            "<div align=\"center\"><a href=\"#\" onclick=\"openBox('void.php?par[mode]=print_dokumen&par[id_tagihan]=$r[id_tagihan]" . getPar($par, "mode, id_tagihan") . "',800,425);\" title=\"PRINT SP3\" class=\"detail\"><span>detail</span></a></div>",
            "<div align=\"center\" $background>".$noTiket."</div>",
            ...
            <table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dataList\">
                <thead>
                    <tr>
                        ...
                        <th style=\"vertical-align: middle; min-width: 40px;\">sp3</th>
