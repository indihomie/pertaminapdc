...
function tambah(){
	...
	$sql = "insert into doc_file (id, idRencana, namaFile, file, keterangan, kategori_dokumen, kegiatan_dokumen, createDate, createBy) values ('$id', '$inp[kegiatan_dokumen]','$inp[namaFile]','$inp[file]','$inp[keterangan]','$inp[kategori_dokumen]','$inp[kegiatan_dokumen]','".date('Y-m-d H:i:s')."','$cUsername')";
	// echo $sql;
	 //die();
	db($sql);
...
function ubah(){
	...
	$sql = "update doc_file set namaFile = '$inp[namaFile]', file = '$inp[file]', keterangan = '$inp[keterangan]', kategori_dokumen = '$inp[kategori_dokumen]',kegiatan_dokumen = '$inp[kegiatan_dokumen]', updateDate = '".date('Y-m-d H:i:s')."', updateBy = '$cUsername' where id = '$par[id]'";
	db($sql);
...
<div id=\"contentwrapper\" class=\"contentwrapper\">
	...
	<form action=\"\" method=\"post\" id = \"form\" class=\"stdform\" onsubmit=\"return false;\">
		...
		";
		if(isset($menuAccess[$s]["add"])) $text.="<a onclick=\"openBox('popup.php?par[mode]=add".getPar($par,"mode")."',725,450);\" href=\"#\" class=\"btn btn1 btn_document\"><span>Tambah Data</span></a>";
		$text.="
	...
	// echo $sLimit;
		...
		$sql = " SELECT t1.*,date(t1.createDate) as tanggal from doc_file t1 join doc_rencana t2 on t1.idRencana = t2.id $filters order by $orderBy $sLimit ";
		// echo $sql;
...
if(isset($menuAccess[$s]["delete"]))
			...
			$data=array(
				...
				"<div align=\"center\">$r[createBy]</div>",	
	...
	function xls3(){
		...
		$judul = $arrTitle[$s];
		$field = array("no", "dokumen", "tanggal", "user", "kategori");
		$width = array(5, 30, 30, 30, 30);
		...
		$sql = " SELECT t1.*,date(t1.createDate) as tanggal,t2.idKategori,t2.aktifitas from doc_file t1 join doc_rencana t2 on t1.idRencana = t2.id $filters";
		...
		exportXLS($direktori, $namaFile, $judul, 5, $field, $data, false, "", "", $width);	
	}
  
	function xls(){		
	global $db,$par,$arrTitle,$arrIcon,$cName,$menuAccess,$fExport,$cUsername,$s,$cID,$areaCheck,$cUsername;
	...
	$objPHPExcel->getActiveSheet()->setCellValue('E4', "KATEGORI");
	...
	while($r=mysql_fetch_array($res)){			
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$rows, getTanggal($r[tanggal]));
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, $cUsername);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$rows, $arrMaster[$r[idKategori]]);
		...
		<table style=\"width: 100%\">
				<tr>
					<td width=\"50%\">
						...
						<span class=\"field\">$r[createDate]&nbsp;</span>
					</td>
					<td width=\"50%\">
						<label class=\"l-input-small\" style=\"width:50%\">Input By</label>
						<span class=\"field\">$r[createBy]&nbsp;</span>		
					</td>
				</tr>
			</table>
			...
			<table style=\"width: 100%\">
				<tr>
					...
					<td width=\"50%\">
						...
						<span class=\"field\">$r[updateBy]&nbsp;</span>		
					</td>
				</tr>
			</table>
